...
/*   Updated: 2025/05/20 18:46:09 by ulmagner         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	assign_color(t_color color, char c, t_all *all, int *x)
{
	*x += 20;
	...
	if (c == 'C' && !all->player.access.status)
...
void	minimap(t_all *all)
{
	...
	while (row)
	{
		...
		while (col)
		{
			color = assign_color(color, col->i, all, &x);
