...
// unsigned int(color) = (all->floor.r << 16) | (all->floor.g << 8)
	// | all->floor.b;
...
void	floor_ceiling_raycasting(t_all *all, t_raycasting *r,
								t_map *cp, t_player *p)
{
	float (current_dist) = all->window.main_h
		/ (2.0 * r->y - all->window.main_h);
	...
	int (color_ceiling) = get_pixel_color(&all->tex.tiles[0][0][0],
			tex_x, tex_y);
